...
export function nukeCommand(program: Command) {
  program
    .command("nuke")
    .description(
      "Delete all node_modules directories in the current directory and subdirectories"
    )
    ...
    .action(async (options: { yes?: boolean; dryRun?: boolean }) => {
      try {
        ...
        for (const dir of nodeModulesDirs) {
          ...
          totalSize += size;
          console.log(
            `  ${chalk.cyan(relativePath)} ${chalk.gray(`(${formatBytes(size)})`)}`
          );
        }
        ...
        if (!options.yes) {
          ...
          const answer = await new Promise<string>((resolve) => {
            rl.question(
              chalk.red(
                "Are you sure you want to delete all these directories? (y/N) "
              ),
              resolve
            );
          });
